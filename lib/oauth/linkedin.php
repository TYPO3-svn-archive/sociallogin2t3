...
class LinkedIn {
  ...
  function __construct($consumer_key, $consumer_secret, $oauth_callback = NULL) {
    ...
    $this->consumer = new Renamed_OAuthConsumer($consumer_key, $consumer_secret, $this->oauth_callback);
    $this->signature_method = new Renamed_OAuthSignatureMethod_HMAC_SHA1();
  ...
  function getRequestToken() {
    ...
    $request = Renamed_OAuthRequest::from_consumer_and_token($consumer, NULL, "GET", $this->request_token_path);
    ...
    $this->request_token = new Renamed_OAuthConsumer($response_params['oauth_token'], $response_params['oauth_token_secret'], 1);
    }
  ...
  function getAccessToken($oauth_verifier) {
    $request = Renamed_OAuthRequest::from_consumer_and_token($this->consumer, $this->request_token, "GET", $this->access_token_path);
    ...
    $this->access_token = new Renamed_OAuthConsumer($response_params['oauth_token'], $response_params['oauth_token_secret'], 1);
  }
  
  function getProfile($resource = "~") {
    $profile_url = $this->base_url . "/v1/people/" . $resource;
    $request = Renamed_OAuthRequest::from_consumer_and_token($this->consumer, $this->access_token, "GET", $profile_url);
  ...
  function setStatus($status) {
    ...
    echo $xml . "\n";
    $request = Renamed_OAuthRequest::from_consumer_and_token($this->consumer, $this->access_token, "PUT", $status_url);
    $request->sign_request($this->signature_method, $this->consumer, $this->access_token);
  ...
  function search($parameters) {
    ...
    $request = Renamed_OAuthRequest::from_consumer_and_token($this->consumer, $this->access_token, "GET", $search_url);
    $request->sign_request($this->signature_method, $this->consumer, $this->access_token);
    $auth_header = $request->to_header("https://api.linkedin.com");
    if ($debug) {
